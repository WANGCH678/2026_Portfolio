...
- **문제:** 영업사원이 하루 수십 개 판매점을 방문 — 방문 상태(완료·진행중·예정·취소)와 순서 관리가 종이·기억에 의존
- **해결:** 드래그 핸들로 경로 순서를 직접 재정렬할 수 있게 구현, 4가지 방문 상태를 컬러 바 + 배지로 구분, 업무시작·종료 버튼으로 하루 흐름을 앱 안에서 완결
- **구현:** 터치 최적화 모바일 레이아웃, 드래그 경로 재정렬, 실시간 방문 상태 업데이트
- **결과:** 영업사원이 이동 중에도 방문 순서를 조정하고 진행 상태를 실시간으로 확인 가능
- **배운 점:** 모바일은 정보량보다 액션 진입점(버튼 위치·크기·우선순위)의 명확성이 핵심이라는 것을 체감

---

*관련 페이지: [[career-nulenn]], [[project-sk-safety]], [[project-tongilnanum]]*
*최종 수정: 2026-07-21*
