...
- **문제:** 영업 담당자가 수십 개의 해외 안건 상태와 수주 실적을 분산된 엑셀로 관리
- **해결:** 안건 생애주기(등록→검토→수주→완료)를 CRM으로 통합, 차트·캘린더·테이블 복합 대시보드
- **구현:** 다중 탭 SPA 구조, 피벗 테이블 렌더링, 복합 필터 시스템
- **결과:** 영업팀이 단일 시스템에서 안건 현황·일정·실적을 통합 관리
- **배운 점:** 엔터프라이즈 CRM에서 복잡한 데이터 구조를 테이블+차트 조합으로 표현하는 패턴
