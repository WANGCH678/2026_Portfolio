...
> 여러 CDU(증류 유닛) 단위의 수치를 카드 그리드로 배치하고 Trend Monitoring·
Data 조회를 탭으로 구성한 설계는 "복잡한 정보 구조를 사용자가 파악하기 쉽게 정리하는 FE 능력"으로 어필해야 한다.
...
- **문제:** 수십 개 공정 유닛(CDU 등)의 데이터가 흩어져 있어 이상 징후를 즉시 파악하기 어려움
- **해결:** 트리 내비게이션 + 멀티탭 + 컬러 코딩 카드 그리드로 계층 구조 설계, 정상·경고·위험을 색으로 즉시 구분되는 다크 테마 대시보드 구현
- **구현:** 다크 테마 컴포넌트 시스템, 실시간 데이터 바인딩, 게이지/그리드 복합 레이아웃
- **결과:** 운영자가 단일 화면에서 이상 징후를 즉시 포착 가능
- **배운 점:** 데이터 밀도가 높을수록 정보 계층과 시각적 우선순위 설계가 성패를 가른다는 것을 체감
